...
uint32_t blurStrength() {
    bool ok = false;
    const uint32_t v = qEnvironmentVariableIntValue("DTK_BLUR_STRENGTH", &ok);
    return ok ? v : 300;
}
...
DKWinBlurManager::~DKWinBlurManager() {
    const QList<org_kde_kwin_blur *> blurs = m_blurs.values();
...
bool DKWinBlurManager::ensureManager() {
    ...
    if (blurDebug())
        qDebug() << "(DWindow) Blur MGR bound:" << (m_manager != nullptr);
...
void DKWinBlurManager::setBlur(QWindow* window, const QRegion& region) {
    ...
    const uint32_t strength = blurStrength();
    ...
    if (blurDebug())
        qDebug() << "(DWindow) Blur region" << region.boundingRect()
                 << "strength" << strength << "for" << window;
}
